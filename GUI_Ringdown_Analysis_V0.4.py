...
class Window1():
	...
	def __init__(self, master):
		self.master = master
		self.master.title('Ringdown Analysis')
	...
	def Info(self, event = None):
		...
		tx3 = 'June 2012\n\n'
		tx4	= 'Authors:\n\n'
		tx5 = 'José Antonio de la O Serna\n'
	...
	def Check_Data(self, *args):
		...
		if 7 in args:
			...
			if (self.val_opmn3.get() == 'ERA') or (self.val_opmn3.get() == 'MP'):
		...
		if 9 in args:
			...
			if self.val_W1chb2.get() == True:
				...
				try:
					...
					if dwn_spl % 1 > 0:
						tk.messagebox.showerror('Error', 'Downsampling M factor data are not valid.')
						return False
				...
				if dwn_spl < 1:
					tk.messagebox.showerror('Error', 'Downsampling M factor out of range.')
					return False
...
class Window2():
	...
	def Create_Widgets(self, file_path):
		...
		self.ent3.insert(0, int(self.val_W1ent9.get()))
